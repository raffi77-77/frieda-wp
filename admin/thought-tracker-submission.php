...
function thoughtTrackerMenu() {
	add_menu_page(
		'Thought Tracker Submission',
		'Thought Tracker Submission',
		'manage_options',
		'thought-tracker-submission',
		'thoughtTrackerSubmission',
		'dashicons-schedule',
	);
}
add_action('admin_menu', 'thoughtTrackerMenu');

function thoughtTrackerAdminJs($hook) {
	wp_enqueue_script('admin-script', get_template_directory_uri().'/assets/js/admin.js');
}
add_action( 'admin_enqueue_scripts', 'thoughtTrackerAdminJs');

function thoughtTrackerSubmission() {
	$pageLink = site_url().'/wp-admin/admin.php?page=thought-tracker-submission';
	...
	?>
	<div class="wrap">
		<h1 class="wp-heading-inline">Thought Tracker Submission</h1>
